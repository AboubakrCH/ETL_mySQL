...
def get_type(txt):
	...
	with connection.cursor() as cursor:
		...
		for tuple_ in myREGEX:
			...
			regex = tuple_['regex']
			x = re.match(regex.upper(), txt.upper())
			if x : 
	...
	for data in old_lst :
		if data['OLDVALUES'] != None :
			...
			for reg in regex_lst :
				if re.match(reg['regex'].upper(),data['OLDVALUES'].upper()):# i changed REGULAR by regex
					#update value
	...
	for data in old_lst :
		if data['OLDVALUES'] != None :
			...
			if not found :
				...
				test = mycursor2.execute('select OLDVALUES,COUNT(OLDVALUES) as CB FROM DR_CSVFILE_COL_3 WHERE OLDVALUES="{}" GROUP BY OLDVALUES having soundex(OLDVALUES) IN (select soundex(FRENCH) from DDVS)'.format(data['OLDVALUES']))
...
def main():
	...
	print("######  STARTED  ######")
	nbr_column, data = open_csvfile(path_file='/Applications/MAMP/htdocs/ETL_mySQL/ETL/APP/file_uploaded/csvfile.csv',delimiter=';')
	clean_database()
	...
	print("FINISHED IN {} SECONDS".format(round(time.time()-started_time,2)))
